...
function TiltCard({ project, onOpen, featured = false }: { project: Project; onOpen: (p: Project) => void; featured?: boolean }) {
  ...
  return (
    <motion.button
      ...
      className={`group relative flex h-full min-h-[18rem] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white text-left shadow-sm transition-all hover:shadow-2xl dark:border-slate-700 dark:bg-slate-900 ${featured ? "md:col-span-2 md:row-span-2 md:min-h-[30rem]" : ""}`}
    >
